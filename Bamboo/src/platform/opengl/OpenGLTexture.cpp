...
namespace BAMBOO
{
		...
		: _Path(path)
	{
		...
		{
			BB_PROFILE_SCOPE("stbi_load - OpenGLTexture2D::OpenGLTexture2D(path)");
		...
		BB_CORE_ASSERT(internalFormat & outputFormat, "Format not supported!");
	...
	void OpenGLTexture2D::SetData(void * data, uint32_t size)
	{
		...
		BB_CORE_ASSERT(size == _Width * _Height * bpp, "Data must be entire texture!");
