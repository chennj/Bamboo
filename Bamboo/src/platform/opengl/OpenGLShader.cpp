...
namespace BAMBOO
{
	static GLenum ShaderTypeFormatString(const std::string& type)
	{
		...
		BB_CORE_ASSERT(false, "Unknown shader type '{0}'", type);
		return 0;
	}
	...
	std::string OpenGLShader::ReadFile(const std::string & filepath)
	{
		...
		else
		{
			...
			BB_CORE_INFO("Current file directory:{0}", FilePath);
		}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string & source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			BB_CORE_ASSERT(type == "vertex" || type == "fragment" || type == "pixel", "Invalid shader type");
